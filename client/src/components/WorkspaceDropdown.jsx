...
function WorkspaceDropdown() {
    ...
    // Use Clerk's active organization as fallback
    const displayWorkspace = currentWorkspace || (activeOrg ? {
        id: activeOrg.id,
        name: activeOrg.name,
        image_url: activeOrg.imageUrl
    } : null);

    const displayCount = workspaces.length > 0 ? workspaces.length : (userMemberships?.data?.length || 0);
    ...
    // Sync Redux with Clerk's active organization
    useEffect(() => {
        if (activeOrg && (!currentWorkspace || currentWorkspace.id !== activeOrg.id)) {
            dispatch(setCurrentWorkspace(activeOrg.id))
        }
    }, [activeOrg, currentWorkspace, dispatch])
        ...
        <div className="relative m-4" ref={dropdownRef}>
            <button onClick={() => setIsOpen(prev => !prev)} className="w-full flex items-center justify-between p-3 h-auto text-left rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                <div className="flex items-center gap-3">
                    <img src={displayWorkspace?.image_url} alt={displayWorkspace?.name} className="w-8 h-8 rounded shadow" />
                    <div className="min-w-0 flex-1">
                        <p className="font-semibold text-gray-800 dark:text-white text-sm truncate">
                            {displayWorkspace?.name || "Select Workspace"}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-zinc-400 truncate">
                            {displayCount} workspace{displayCount !== 1 ? "s" : ""}
                        </p>
                    </div>
                </div>
                    ...
                    <div className="p-2">
                        ...
                        {userMemberships?.data?.map(({ organization }) => (
                            <div key={organization.id} onClick={() => onSelectWorkspace(organization.id)} className="flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-zinc-800" >
                                ...
                                {(displayWorkspace?.id === organization.id || activeOrg?.id === organization.id) && (
